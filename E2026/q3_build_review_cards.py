...
</style></head><body><header><h1>问题三：附件4解释证据回看</h1>
...
const key=id=>'q3-review-'+id;function saved(id){try{return JSON.parse(localStorage.getItem(key(id)))||{}}catch{return {}}}
...
$('export').onclick=()=>{const rows=[['sample_id','review_decision','review_notes'],...data.map(x=>{const s=saved(x.id);return[x.id,s.decision||'pending',s.notes||'']})];
 ...
 const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='q3_explanation_review.csv';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000)};
